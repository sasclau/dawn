  ...
  return {
    initRVItems: function () {
      if (window.location.search.indexOf('page=') == -1) {
        const rv_items = JSON.parse(localStorage.getItem('melisa__recently-viewed-products'));
        this.init(rv_items);
      }
    },
    ...
    init: function (rv_items) {
      ...
      if (rv_items === null || rv_items.length === 0) {
        ...
        const section_id = document.querySelector('.rv-page-section').dataset.sectionId;
        var productAjaxURL =
    ...
    clearAll: function () {
      ...
      if (rv_div) {
        ...
        rv_div.querySelector('.clear-all-rvp').addEventListener('click', function () {
          rv_div.querySelector('.rv-results').remove();
          rv_div.querySelector('.clear-all-rvp').classList.add('hidden');
          localStorage.removeItem('melisa__recently-viewed-products');
    ...
    skeletonFunction: function (timer) {
      window.setTimeout(function () {
        ...
        if (this.document.querySelector('.rv-page-section-inner')) {
          this.document.querySelector('.rv-page-section-inner').classList.remove('hidden');
        }
      }, timer);
    },
  };
