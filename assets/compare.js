...
var BlsComparePageShopify = (function () {
  return {
    ...
    init: function () {
      const compare_items = JSON.parse(localStorage.getItem('melisa__compare-items'));
      ...
      if (compare_items === null || compare_items.length === 0) {
        ...
        const section_id = document.querySelector('.compare-page-section').dataset.sectionId;
        var productAjaxURL =
          '?section_id=' + section_id + '&type=product&options[unavailable_products]=last&q=id:' + query;
        fetch(`${window.routes.search_url}${productAjaxURL}`)
          ...
          .then((responseText) => {
            ...
            if (compare_items.length !== 0) {
              ...
              const availability = document.querySelector('.compare-row-availability');
              ...
              const bpc = html.querySelectorAll('.compare-page-section > .product-compare');
              if (bpc.length > 0) {
                if (table) {
                ...
                bpc.forEach((el) => {
                  basic.innerHTML += el.querySelector('.compare-row-basic').innerHTML;
                  vendor.innerHTML += el.querySelector('.compare-row-vendor').innerHTML;
                  availability.innerHTML += el.querySelector('.compare-row-availability').innerHTML;
                  size.innerHTML += el.querySelector('.compare-row-size').innerHTML;
                  color.innerHTML += el.querySelector('.compare-row-color').innerHTML;
                  review.innerHTML += el.querySelector('.compare-row-review').innerHTML;
                });
              } else {
                div_no_product.classList.remove('hidden');
              }
...
class CompareRemove extends HTMLElement {
  ...
  onThisClick() {
    ...
    const allThisProductIds = document.querySelectorAll(`compare-remove[data-product-id="${this.productId}"]`);
    let listProductIds = [];
    ...
    if (localListProductIds && allThisProductIds.length > 0) {
      ...
      const product_remove = document.querySelectorAll(`[data-product-handle="${this.productHandle}"]`);
